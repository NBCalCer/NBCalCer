...
calc_SONR_new <- function(YMAX,CRPRICE,FERT_PRICE,YMULT,RET_SRO,RET_GRW,
  ...
  XIN <- c(seq(-500,-1,by=1),seq(0,400,by=0.01))
...
benef_at_new <- function(X,YMAX,CRPRICE,FERT_PRICE,YMULT,RET_SRO,RET_GRW,
  ...
  # For Marine
  NLOSS <- (1-RET_SRO)*X+(1-RET_GRW)*NSURP_CALC
...
get_baseshp_fast <- function(BASEMAPS_FOLDER,BASEMAPS_NAME){
  ...
  if(!exists(aux)){
    ...
    auxt <- paste0(aux,"<<- readShapeSpatial(paste0(BASEMAPS_FOLDER,BASEMAPS_NAME,'.shp'))") 
    eval(parse(text=auxt))
  }
  auxt <- paste0("d <- ",aux)
  eval(parse(text=auxt))
...
get_region_name <- function(REGION_CODE){
  ...
  if(length(region_name)>1){ unexpected_error }
...
# print(paste0("id: ",id," m_years_act:",m_years_act))  
  if(id=="YMAX"){
   param_act <- "yw" 
   if(SYSTEM=="irrigated"){
     param_act <- "yp"
  ...
  for(i in 1:length(files_act)){
    ...
    if(!is.na(ract)){ 
      ...
      if(is.na(as.numeric(res)*MULT)){ return(res) } # quiza estemos devolviendo texto
      return(res*MULT)
    }
  }
...
# print(s)
  # If there is some error or not found it will tell it
  # No errors will return NA
  # Errors found and corrected then will return a corrected value
  # print(s) 
  if(column=="data"){
...
get_long_term_N_response_curve <- function(crop,CURVE_TYPE){
  # CURVE_SEL <- str_to_upper(CURVE_SEL)
  AUX <- rep(NA,3)
...
calculate_yields_curve_george  <- function(cpA,cpB,cpC,NAV,YMAX){
  ...
  for(i in 1:length(cpA)){
    ...
    if(!is.na(NAV)){
      ...
      YPRED <- (pA+pB*(0.99^NAV)+pC*NAV)*YMAX
      resY <- c(resY,YPRED)
...
calculate_yields_curve_quadratic <- function(cpA,cpB,NAV,YMAX,crop,MAXIMUM_YIELD_DECREASE_WITH_INCR_N,AVOID_YIELD_DECREASE_WITH_INCR_N){
  ...
  for(i in 1:length(cpA)){
    ...
    if((!AVOID_YIELD_DECREASE_WITH_INCR_N) || is.na(NAV) || is.na(YMAX)){
      YPRED <- min(1,NAV*pA + (NAV^2)*pB)*YMAX
      resY <- c(resY,YPRED)
      next
    }
  ...
  for(i in 1:length(NAMES)){
    ...
    if((get_config_parameter("PROCESS_COMBINATIONS",txt = F)>0) |
       (exists("PROCESS_COMBINATIONS_WORLD_COUNTRIES") && 
        NAMES[i]=="world_countries" & PROCESS_COMBINATIONS_WORLD_COUNTRIES==F)){
        ...
        for(rg in aux){
          ...
          for(sy in unique(dres_act$system)){
            for(cr in unique(dres_act$crop)){
              ...
              shpact@data <- left_join(shpact@data,dactrg,selcol)
              writeSpatialShape(shpact, paste0(dir_maps_act,"map_",isocode,"_",cr,"_",sy))
        ...
        if(DATASETS[i]=="dcountry"){
          for(sy in unique(dres_act$system)){
            for(cr in unique(dres_act$crop)){
              ...
              dmap <- joinCountryData2Map(dact, joinCode = "ISO2",nameJoinColumn = "country_code",verbose =F)
             
              dmap@data$country_code[which(dmap@data$NAME_SORT=="Namibia")] <- "NA"
              writeSpatialShape(dmap, paste0(dir_maps_act,"map_",cr,"_",sy))
            }
          }
        }else{
          # World regions
          for(sy in unique(dres_act$system)){
            for(cr in unique(dres_act$crop)){
              ...
              # regions_files
              # > regions_files
              # [1] "world_regions_inms"  "world_regions_image" "world_regions_agmip"
              # NAMES[i]
              
              # shpact <- get_baseshp_fast(BASEMAPS_FOLDER,REGION_CLASSIFICATION)
              ...
              shpact@data <- left_join(shpact@data,dact,"region_cod")
              writeSpatialShape(shpact, paste0(dir_maps_act,"map_",cr,"_",sy))
