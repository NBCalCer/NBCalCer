...
NBCalCer_run <- function(CONFIG_FILE, RESULTS_FOLDER,SIMULATION_ID=NA,SUMMARY_FILE=NA,CALCULATE_SONR=F,
  ...
  YEAR_USD <- get_config_parameter("YEAR_USD")
  ...
  for(SYSTEM in SYSTEMS){
      ...
      for(CROP in CROPS){
        ...
        if(!is.na(YMAX_IR) & !is.na(YMAX_RF) & AREA_ACT>50000 &
          ...
          # NFERT <- NA # Not show the results if not enough area or information is available
        }
        ...
        NMAN_EF <- min(NMAN,NMAN_MAX)*FEV_MANURE
        ...
        NLOSS <- (1-RET_SRO)*(NFERT+min(NMAN,NMAN_MAX))+(1-RET_GRW)*NSURPLUS
    ...
    for(cc in coun_details){
      ...
      for(cr in unique(dst$crop)){
        ...
        for(sy in unique(dst$system)){
          ...
          # ya tengo las sumas y las ponderadas, ahora selecciono la adecuada
  ...
  # pasos
  # coger el ndep que es el deposito sobre areas naturales total
  # coger la volatilizacion de amonia y la de manure asumiendo un 20% del manure y luego aplicar el valor
  # de un 45% que cae de nuevo en areas naturales
  # restarlo para obtener el deposito de otras fuentes que no sean cereales
  # con cada valor nacional hacer el algoritmo de la tabla de hans para repartirlo proporcionalmente
  ...
  # deflate the prices 
  print("Deflating costs to baseyear...")
  for(i in 1:nrow(dres)){
    dres$abs_msa_cost[i] <- deflate_consumer_price(dres$country_code[i],YEAR,YEAR_USD,dres$abs_msa_cost[i])
    dres$abs_marine_cost[i] <- deflate_consumer_price(dres$country_code[i],YEAR,YEAR_USD,dres$abs_marine_cost[i])
    dres$abs_pm25_nh3_cost[i] <- deflate_consumer_price(dres$country_code[i],YEAR,YEAR_USD,dres$abs_pm25_nh3_cost[i])
  }
  ...
  if(CALCULATE_SONR){
    ...
    for(cnt in 1:nrow(dres)){
      ...
      # Recovering all the needed value
